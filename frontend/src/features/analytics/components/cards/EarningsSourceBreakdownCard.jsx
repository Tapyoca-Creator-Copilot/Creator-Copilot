...
import { buildEarningSourceData } from "@/features/analytics/utils/categoryBreakdown";
import { currencyFormatter } from "@/features/analytics/utils/formatters";
import { getTimeRangeLabel } from "@/features/analytics/utils/graphTimeRange";
import { cn } from "@/lib/utils";
import { List, ListItem } from "@tremor/react";
import { useMemo } from "react";
import { Cell, Pie, PieChart, ResponsiveContainer, Tooltip } from "recharts";

const EarningsSourceBreakdownCard = ({
  projectId,
  earnings = [],
  isLoading = false,
  timeRange = "month",
  ...
  const currency = useMemo(
    () => projectCurrency || earnings.find((earning) => earning?.project?.currency)?.project?.currency || "USD",
    [earnings, projectCurrency]
  );

  const donutData = useMemo(() => buildEarningSourceData(earnings), [earnings]);
    ...
    return (
      <Card className="border-black/5 dark:border-white/10 bg-card">
        <CardContent className="pt-6 text-sm text-muted-foreground">
          Select a project to view {timeRangeLabel} earning sources.
        </CardContent>
      </Card>
    );
  }
  ...
  if (isLoading) {
    return (
      <Card className="border-black/5 dark:border-white/10 bg-card">
        <CardContent className="pt-6 text-sm text-muted-foreground">
          Loading earning source breakdown...
        </CardContent>
      </Card>
    );
  }
  ...
  if (!donutData.rows.length) {
    return (
      <Card className="border-black/5 dark:border-white/10 bg-card">
        <CardContent className="pt-6 text-sm text-muted-foreground">
          No earning sources found for this {timeRangeLabel} view yet.
        </CardContent>
      </Card>
    );
  }
  ...
  return (
    <Card>
      <CardHeader className="space-y-1">
        <CardTitle>Total earnings by source</CardTitle>
        <p className="text-sm text-muted-foreground">
          {currencyFormatter(donutData.totalAmount, currency)} across {donutData.rows.length} sources
        </p>
      </CardHeader>
...
export default EarningsSourceBreakdownCard;
